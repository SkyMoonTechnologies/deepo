...
const Card = React.forwardRef<React.ElementRef<'div'>, React.ComponentProps<'div'>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'rounded-xl border border-border/80 bg-card/95 text-card-foreground shadow-[0_16px_40px_-28px_rgba(15,23,42,0.6)]',
        className,
      )}
      {...props}
    />
...
const CardTitle = React.forwardRef<React.ElementRef<'h3'>, React.ComponentProps<'h3'>>(
  ({ className, ...props }, ref) => <h3 ref={ref} className={cn('text-xl font-semibold tracking-tight', className)} {...props} />,
);
CardTitle.displayName = 'CardTitle';

const CardDescription = React.forwardRef<React.ElementRef<'p'>, React.ComponentProps<'p'>>(
